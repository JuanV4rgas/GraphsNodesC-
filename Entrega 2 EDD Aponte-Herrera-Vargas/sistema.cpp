    ...
    // Comandos de cuatro o más argumentos
      ...
      if (comandos[0] == "segmentar") {
        if (comandos[1].substr(comandos[1].size() - 4) != ".pgm") {
          ...
          std::cout << "Usted ha llamado correctamente la función segmentar"
                    << std::endl;
        }
      } else if (comandos[0] == "proyeccion2d") {
...
void Sistema::codificarImagen(std::string nombre_archivo_huf) {
  ...
  std::priority_queue<NodoHuffman*, std::vector<NodoHuffman*>, NodoHuffman::Comparador> cola;
  ...
  std::priority_queue<NodoHuffman*, std::vector<NodoHuffman*>, NodoHuffman::Comparador> copia = cola;
  while (!copia.empty()) {
      NodoHuffman* nodo = copia.top();
      std::cout << nodo->obtenerDato() << ": " << nodo->obtenerFrec() << std::endl;
      copia.pop();
  }
...
std::cout << "\nArchivo " << nombre_archivo_huf << " generado correctamente.\n";

}


void Sistema::decodificarArchivo(std::string nombre_archivo_huf, std::string nombre_imagen_pgm) {
  std::ifstream archivo(nombre_archivo_huf.c_str(), std::ios::binary);
  if (!archivo.is_open()) {
      std::cout << "Error: no se pudo abrir el archivo " << nombre_archivo_huf << std::endl;
      return;
  }
  ...
  if (archivo.fail()) {
      std::cout << "Error: formato inválido del archivo (encabezado incompleto).\n";
      return;
  }
  ...
  for (int i = 0; i <= maxIntensidad; i++) {
      archivo.read(reinterpret_cast<char*>(&frecuencias[i]), sizeof(unsigned long));
      if (archivo.fail()) {
  ...
  if (raiz->obtenerIzq() == nullptr && raiz->obtenerDer() == nullptr) {
      ...
      if (!salida.is_open()) {
          std::cout << "Error: no se pudo crear el archivo de salida " << nombre_imagen_pgm << "\n";
          return;
      }
      ...
      salida.close();
      std::cout << "Imagen decodificada correctamente (caso único símbolo) en: " << nombre_imagen_pgm << "\n";
      return;
  }
  ...
  salida.close();
  std::cout << "Imagen decodificada correctamente y guardada en: " << nombre_imagen_pgm << "\n";
}
